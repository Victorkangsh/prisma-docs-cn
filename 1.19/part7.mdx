...
如果`migrations`被设置为TRUE;则部署Prisma服务将迁移已连接数据库的结构。在这些情况下，Prisma CLI将是基于服务的数据模型管理数据库结构的主要接口：
...
<Code>{`
docker pull prismagraphql/prisma:1.19-beta
`}</Code>
...
<Code>{`
docker pull prismagraphql/prisma:1.19-alpha
`}</Code>
